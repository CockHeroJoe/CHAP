...
Download the [installer]("https://github.com/CockHeroJoe/CHAP/builds/artifacts/master/raw/target/msiPackage/chap-x64.msi?job=build") and run it.
...
- `duration`: The duration in seconds of the round (should match song length)
  - **NOTE**: This is not required (and is ignored) if `bmcfg` is set
...
- `-n` or `--name`: The output basename for generated video(s), default "Random XXXX"
- `-v` or `--versions`: The number of [versions](#versions) of each clip, default 1 (not displayed)
- `-r` or `--raw`: Output videos as "raw" `.avi` files with `png` codec
...
- `-t` or `--threads`: Sets number of concurrent threads to process rounds with
...
- `-e` or `--execute`: Skip main GUI and immediately execute assembly program
